...
### §5.3 覆盖纪律臂(2026-07-08 深夜)与"守恒的长尾"

**实现**:`DEEPREAD_COVERAGE=1`(`Config.coverage_discipline`,默认关,pin 测试)
给 SYSTEM_PROMPT 追加两条规则:①点名对象逐一核对已检已读才许作答;
②搜不到禁止原词复搜,换角度重构(专名/英文/上位概念/配套 *KEYWORD),
两次无果才许带缺口作答。全库 4661 篇 × 120 题,与 0.8935 基线唯一差异 = 该块。

**插曲(运维事故,已防御化)**:首轮跑数全灭(0.058)——aiberm 某分发商把工具
schema 名改写成 `Compat<CamelCase><6hex>`,模型照调、ToolBox 全拒。对照探针证明
与纪律块无关。已加 `tools.resolve_tool_name` 还原层(执行/日志/summarize 特判
三处接线,含测试),毒数据归档 `runs/multidoc/incident_20260708_mangled/`。
**运维铁律:跑批前先 1 题探针,验 rich 日志的工具名与 seen_docs>0。**

**净通道结果**:

| | mean_score | 漏检题 | 逐题 | token/题 |
|---|---|---|---|---|
| 基线 | 0.8935 | 7 | — | 57,064 |
| 覆盖纪律 | **0.8628(−0.031)** | **11**(修 5 坏 9) | 28 胜 34 负 | **102,512(+80%)** |

靶子再次全中:item 90 0.50→**1.00**、106 0.61→0.94、49→1.00、23→0.94;
但强迫逐项补检把迭代/上下文预算挤占到别处——9 个**新**漏检出现,成本近翻倍。

**元发现(本战役最重要的一条)**:三个独立单杆干预在同一基准上呈现同一形态——

| 干预 | 靶子修复 | 新增损伤 | 净效应 |
|---|---|---|---|
| enricher 换强模型(glm) | — | — | −0.040 |
| 关键词加密(kw15,全库) | 修 5/7 漏检 | 新增 5 漏检 | −0.020 |
| 覆盖纪律 prompt | 修 5/7 漏检 | 新增 9 漏检,+80% token | −0.031 |

**在 0.89+ 水位上,残余误差质量在单杆干预下近似守恒**:每个针对性修复都命中
诊断病例,并在别处诱发等量或更大的新长尾。这与 94 题 v3 的 0.83 高原结论
(四轴 Pareto 饱和)在新基准上再次成立,且这次是用三把不同的刀独立证明的。
路线含义:继续在这一水位买分需要**组合式/条件触发式**干预(如仅多对象题触发
纪律、重构限一次),或接受 0.89 为该配置的工作点;两个 flag 均维持默认关。
